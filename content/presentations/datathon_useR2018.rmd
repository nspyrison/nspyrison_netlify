...
load(file = "./data_useR2018_datathon/samp_5k_of3m.rda") # loads df 'samp'

GGally::ggpairs(data = samp[3:ncol(samp)])
skimr::skim(samp)
```
...
```{r results="hold",fig.width=9}
samp <- mutate(samp, decade=substring(year, 1, 3))
xymap2 <- 
  ggplot(samp, aes(y = decimalLatitude, x = decimalLongitude, frame = decade),
         alpha = .1) + coord_fixed() + geom_jitter(width = .5, height = .3) + theme_minimal()
